export function formatTable(headers, rows, maxCellWidth = 22) {
    const normalizedHeaders = headers.map((header) => normalizeCell(header, maxCellWidth));
    const normalizedRows = rows.map((row) => headers.map((_, index) => normalizeCell(row[index] ?? "", maxCellWidth)));
    const widths = normalizedHeaders.map((header, index) => Math.max(header.length, ...normalizedRows.map((row) => row[index]?.length ?? 0)));
...
export function formatSignificant(value, significantDigits = 4, preserveTrailingZeros = false) {
    if (!Number.isFinite(value))
        return String(value);
    if (value === 0)
        return preserveTrailingZeros ? value.toPrecision(significantDigits) : "0";
