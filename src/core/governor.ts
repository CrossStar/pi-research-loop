...
const EXPLICIT_BROAD_WORK =
  /\b(?:all tests|full test suite|run (?:the )?entire test|repository[- ]wide|repo[- ]wide|checksum|sha256|exhaustive benchmark)\b|全部测试|完整测试|全量测试|运行.*测试|整个仓库.*(?:测试|格式化)|校验和|完整基准测试/i;
...
export function evaluateResearchFidelity(toolName: string, input: unknown, userPrompt: string): GateDecision {
  ...
  const reason =
    "This reproduction reduces samples, steps, seeds, or repeats without approval. Tell the user the original and proposed settings, why they would change, and how that affects the result, then obtain explicit approval.";
  return {
    block: true,
    reason,
    approval: {
...
function preview(value: string, maximum = 600): string {
...
const MODE_GUIDANCE: Record<Exclude<WorkMode, "experiment">, string> = {
  ...
  exploration:
    "Read only the code and materials relevant to the current objective. Trace the necessary behavior and return the findings directly with useful file references. Switch to Experiment before empirical work.",
};

function experimentGuidance(experiment?: ExperimentContext): string {
